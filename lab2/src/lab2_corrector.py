...
class Lab2Corrector():
    ...
    def compile_student_code(self, aluno_path):
        if os.path.isdir(aluno_path):
            ...
            try:
                ...
                raise CompilationError(self, "Codigo de saida: {e.returncode}\n")
    ...
    def check_fopen_path(self, student_code):
        pattern_entrada = r'fopen\s*\(\s*"entrada1\.txt"\s*,\s*".*?"\s*\)'
        pattern_saida = r'fopen\s*\(\s*"Lab1_[a-zA-Z0-9_]+\.txt"\s*,\s*".*?"\s*\)'
